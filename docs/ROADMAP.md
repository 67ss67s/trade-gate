...
| **Strategy Lab** | planned | Pre-registered experiments: a parameter change states its expected effect before it runs, is replayed on recorded episodes, and only measured wins are promoted. | Strategy versions (`strategies.ts`, `backtest.ts`) |
